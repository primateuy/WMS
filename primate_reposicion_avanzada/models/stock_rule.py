...
class StockRule(models.Model):
    ...
    def create_iwt_lines(self, channel, procurements, place='iwt'):
        ...
        - Productos con múltiplo: la cantidad se redondea según el
          multiplo_rounding_method del canal (por defecto hacia arriba), salvo que el
          resultado supere el stock disponible en origen, caso en el que baja al múltiplo
          inferior (floor de rescate). El tope es una restricción dura, no una preferencia:
          así la IWT nunca nace con una cantidad que stock_multiplos_validation vaya a
          rechazar en action_assign.
        ...
        for product, qty in product_dict.items():
            ...
            if multiple > 1:
                order_qty = product_obj_rec.primate_round_to_multiple(
                    order_qty,
                    rounding_method=channel.multiplo_rounding_method or 'ceil',
                    max_qty=net_on_hand,
                )
                if order_qty <= 0:
