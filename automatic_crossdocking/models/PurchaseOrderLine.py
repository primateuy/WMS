...
class PurchaseOrderLine(models.Model):
    ...
    def _get_qty_received_from_incoming_pickings(self):
        ...
        main_reception_picking = self.env['stock.picking'].search([
            ('origin', 'like', self.order_id.name),
            ('picking_type_id.code', '=', 'incoming'),
            ('state', '=', 'done'),
            ('origin', 'ilike', 'Recepción'),  
        ], limit=1)
        
        if main_reception_picking:
            moves_in_main_picking = self.env['stock.move'].search([
                ('picking_id', '=', main_reception_picking.id),
